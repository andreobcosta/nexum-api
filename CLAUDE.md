...
| Commit estável | bb42ac6 / B10 — PDF ABNT |
...
|---------|-----------------|--------|
| server.js | Entry point — rotas, CORS, SPA fallback, rate limit, health check | Atualizado Sprint 1+3 |
...
- `motor_config` — configuração do motor: doc `global` com limiares e parâmetros (`set({ merge: true })`)
...
- [x] C2 ✓: pdf-extractor.js prompt específico por instrumento — detectarInstrumento() + promptPorInstrumento() (bec528a)
...
| `motor_config` usa `set({ merge: true })` — atualização parcial sem sobrescrever o doc | Permite salvar campos individuais sem destruir configurações não enviadas |
...
- **Seed `system_prompts/active` pendente** — doc `active` na collection `system_prompts` no Firestore ainda vazio — AdminPage exibe tela em branco sem esse dado; executar seed antes de usar Admin em produção
